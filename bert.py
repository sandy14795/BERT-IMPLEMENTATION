...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    if attention_mask is not None:
        scores = scores.masked_fill(attention_mask == -1e4, -1e4) #to_do
        scores += attention_mask
        
    norm_scores = F.softmax(scores, dim=-1)
    attn = torch.matmul(norm_scores, value)  
    
    attn = attn.transpose(1, 2).contiguous().view(scores.shape[0], -1, self.num_attention_heads * self.attention_head_size)
    return attn
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    """
    # todo
    x = ln_layer(dropout(dense_layer(output)) + input)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    x = self.self_attention.forward(hidden_states, attention_mask)
    # add-norm layer
    x_inp = self.add_norm(hidden_states, x, self.attention_dense,  self.attention_dropout, self.attention_layer_norm)
    # feed forward
    x = self.interm_af(self.interm_dense(x_inp))
    # another add-norm layer
    x = self.add_norm(x_inp, x, self.out_dense, self.out_dropout, self.out_layer_norm)
